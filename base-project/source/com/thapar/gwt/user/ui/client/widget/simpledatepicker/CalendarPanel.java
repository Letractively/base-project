...
/*
 * CalendarPanel extends AbsolutePanel and is the main panel which displays the current month and its days. All the weekdays are selectable items. Clicking on any weekday makes the selection and the date is displayed in the textbox. All the
 * weekends are grayed out and doesnot allow selection
 */
public class CalendarPanel extends AbsolutePanel {
  ...
  private IDatePickerDateProvider dateProvider = null;
   ...
   * @param DatePicker
   */
  public CalendarPanel(DatePicker datePicker, IDatePickerDateProvider dateProvider) {
   ...
   * buildMonthHeader
   * 
   * Displays the header of the month with all the days of the week spelled out
   */
   ...
   * buildMonthBody
   * 
   * Calculates the number of days in the current month and the first start day. Then displays all the dates of that month. Weekdays are made selectable while weekends are grayed out and cannot be selected
   */
  ...
  // html = new HTML("<font class='currentDate'>" + selectedDate + "</font>");
  // monthBody.setWidget(row, col, html);
  // } else {
  // if (this.isWeekendSelectable()) {
  // html = new HTML("<font class='holidaySelectable'>" + selectedDate + "</font>");
  // monthBody.setWidget(row, col, html);
  // } else {
  // html = new HTML("<font class='holiday'>" + selectedDate + "</font>");
  // monthBody.setWidget(row, col, html);
  // }
  // }
  // } else {
  ...
  // monthBody.setWidget(row, col, html);
  // } else {
  // html = new HTML("<font class='calendarDate'>" + selectedDate + "<font>");
  // monthBody.setWidget(row, col, html);
  // }
  // }
  // } else {
  ...
  // }
  // }
  // if (dateProvider != null && dateProvider.shouldHighlightDate(date)) {
  // DOM.setStyleAttribute(html.getElement(), "background", "blue");
  // }
  // }
  // }
  // }
  protected void buildMonthBody(Date date, FlexTable monthBody) {
    ...
    for (int row = 1; row < 7; row++) {
      for (int col = 0; col < 7; col++) {
        try {
          DOM.setStyleAttribute(monthBody.getFlexCellFormatter().getElement(row, col), "background", "");
          DOM.setStyleAttribute(monthBody.getFlexCellFormatter().getElement(row, col), "color", "black");
        } catch (Exception e) {
        }
        myDate = new Date(myDate.getTime());
        myDate.setDate(dayCount + 1);
        boolean dateHasData = dateProvider != null && dateProvider.shouldHighlightDate(myDate);
        if (row == 1 && col < startWeekDay) {
          monthBody.setText(row, col, "");
          cellValues[row][col] = "";
        } else {
          if (numDays > dayCount) {
            ...
            if (col == 0 || col == 6) {
              if (true == isSelectedDate(date, selectedDate)) {
                ...
                DOM.setStyleAttribute(monthBody.getFlexCellFormatter().getElement(row, col), "background", "blue");
                DOM.setStyleAttribute(monthBody.getFlexCellFormatter().getElement(row, col), "color", "white");
              } else {
                if (this.isWeekendSelectable()) {
                  monthBody.setHTML(row, col, "<font class='holidaySelectable'>" + selectedDate + "</font>");
                  addDataStyle(monthBody, row, col, dateHasData, myDate);
                } else {
  ...
  public void addDataStyle(FlexTable monthBody, int row, int col, boolean dateHasData, Date renderDate) {
    if (dateProvider != null && !dateHasData && renderDate.before(new Date())) {
      monthBody.getFlexCellFormatter().getElement(row, col).setTitle("Missing Data");
      DOM.setStyleAttribute(monthBody.getFlexCellFormatter().getElement(row, col), "background", "red");
      DOM.setStyleAttribute(monthBody.getFlexCellFormatter().getElement(row, col), "color", "white");
    } else if (dateProvider != null && dateHasData) {
      monthBody.getFlexCellFormatter().getElement(row, col).setTitle("");
      DOM.setStyleAttribute(monthBody.getFlexCellFormatter().getElement(row, col), "background", "green");
      DOM.setStyleAttribute(monthBody.getFlexCellFormatter().getElement(row, col), "color", "white");
    }
  ...
  /*
   * DateClickListener is a Listener class which implementes the TableListener.
   * 
   * The main panel uses FlexTable to display the dates of the month. This Listener is for the whole Table
   */
  protected class DateClickListener implements TableListener {
    ...
    public void onCellClicked(SourcesTableEvents sender, int row, int col) {
      ...
      /*
       * String dateStr = DateFormatter.formatDate( new Date( currentDate.getYear(), currentDate.getMonth(), Integer.parseInt(this.calPanel.cellValues[row][col]) ), //DateFormatter.MMDDYYYY calPanel.getDateFormat() );
       */
      Date newDate = new Date(currentDate.getYear(), currentDate.getMonth(), Integer.parseInt(this.calPanel.cellValues[row][col]));
      String dateStr = calPanel.getDateFormatter().formatDate(newDate);
   ...
   * isSelecteDate
   * 
   * Checks whether or not the date being printed is the selected date or current date
   */
  private boolean isSelectedDate(Date date, int printedDate) {
    ...
    if (selectedDate != null && selectedDate.getMonth() == date.getMonth() && selectedDate.getYear() == date.getYear() && selectedDate.getDate() == printedDate) {
      isCurrentDate = true;
    }
